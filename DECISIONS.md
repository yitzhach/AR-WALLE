...
- **Scale policy:** latest user request allows gestures. Pinch defaults on; true-size mode disables it. Measurement overlays force scale lock and uncheck pinch. Explicitly enabling pinch hides both overlays. Launch URLs specify scaling on/off. Fresh launch URLs reduce reuse of prior AR state, but cannot guarantee native tracking behavior.
...
- **Editor navigation:** six bottom tabs below the wall preview group artwork, image, display, wall, arrangement and library settings. Only the active settings panel is shown.
...
- **Library:** browser IndexedDB with automatic saving of new uploads, explicit saving after edits, soft delete/undo and JSON backup. No accounts or cloud persistence. Whole scene persistence is outside this iteration.
...
- **Performance and imports:** immutable texture Blobs cache bytes and CRCs through weak references. Library thumbnails are 256-pixel originals cached for the session; original storage is untouched. Backup records are reconstructed from recognized schema fields. Upload batches accept files that fit and report individual failures.
